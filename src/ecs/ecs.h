...
constexpr cmpId_t maxComponentId = static_cast<cmpId_t>(cmp::cmpList::size);
constexpr grpId_t maxGroupId = static_cast<grpId_t>(grp::grpId::_LAST_GRP_ID);
constexpr hdlrId_t maxHandlerId = static_cast<hdlrId_t>(hdlr::hdlrId::_LAST_HDLR_ID);
constexpr sysId_t maxSystemId = static_cast<sysId_t>(sys::sysList::size);

// a template variable to obtain the component id.
template<typename T>
constexpr cmpId_t cmpId = static_cast<cmpId_t>(mpl::IndexOf<T, cmp::cmpList>());

// a template variable to obtain the system id.
template<typename T>
constexpr sysId_t sysId = static_cast<sysId_t>(mpl::IndexOf<T, sys::sysList>());
